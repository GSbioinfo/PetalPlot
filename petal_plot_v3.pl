...
if (@argume < 1){ die "Please provide .txt input file"; }
if (@argume == 2){
...
my $outer_axis=($defaul_im_size[0]/2)*0.95;#490; #radius of outer circle upper limit 
my $inner_axis=($defaul_im_size[0]/2)*0.15;#49; #radius of inner circle lower limit
my @maxmin_lim= (0,1);
my $col="blue";
...
#while (@newtemarray){
foreach my $txxt (@newtemarray){
my @temxsp = @{$txxt};
#print Dumper \@temxsp;
#my @newploypt = splice(splice(@newtemarray, 0,2), 0,2); 
my @temsplic = splice(@temxsp, 0,2);

my @newploypt = splice(@temsplic, 0,2); 
